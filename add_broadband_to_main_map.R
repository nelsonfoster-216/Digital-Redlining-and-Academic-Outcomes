...
# ----------------------
# Step 1: Check if broadband data exists
# ----------------------
broadband_data_path <- "cleveland_broadband_data.rds"
broadband_geojson_path <- "cleveland_broadband_data.geojson"

if(!file.exists(broadband_data_path) || !file.exists(broadband_geojson_path)) {
  stop("Broadband data not found. Please run the broadband_pdf_extractor.R script first.")
}
...
# Copy the GeoJSON file to the target location for the main map
broadband_target_geojson <- "broadband_cleveland.geojson"
file.copy(broadband_geojson_path, broadband_target_geojson, overwrite = TRUE)

message("Created GeoJSON file for broadband data: ", broadband_target_geojson)
