...
<script src="broadband_layer.js"></script>
...
if (!file.exists(redlining_file)) {
  ...
  if (is.null(redlining_data)) {
    ...
    map <- map %>%
      ...
      addLayersControl(
        overlayGroups = c("Redlining Districts", "Schools", "Broadband Speeds (2021)"),
...
map %>% 
  htmlwidgets::onRender("
    function(el, x) {
      ...
      // Store map reference for easy access
      var map = this;
      
      // Define broadband data directly in JavaScript instead of fetching it
      console.log('Creating broadband data directly...');
      ...
      // Define GeoJSON data directly to avoid fetch/CORS issues
      var broadbandData = {
        'type': 'FeatureCollection',
        'features': [
          {
            'type': 'Feature',
            'properties': {'speed_category': '0-9 Mbps', 'color_code': '#d73027'},
            'geometry': {
              'type': 'Polygon',
              'coordinates': [[
                [-81.68, 41.5], [-81.67, 41.52], [-81.65, 41.52],
                [-81.64, 41.49], [-81.67, 41.48], [-81.68, 41.5]
              ]]
            }
          },
          {
            'type': 'Feature',
            'properties': {'speed_category': '10-24 Mbps', 'color_code': '#f46d43'},
            'geometry': {
              'type': 'Polygon',
              'coordinates': [[
                [-81.73, 41.46], [-81.75, 41.48], [-81.74, 41.52],
                [-81.71, 41.53], [-81.69, 41.5], [-81.71, 41.47], [-81.73, 41.46]
              ]]
            }
          },
          {
            'type': 'Feature',
            'properties': {'speed_category': '25-49 Mbps', 'color_code': '#fdae61'},
            'geometry': {
              'type': 'Polygon',
              'coordinates': [[
                [-81.8, 41.42], [-81.85, 41.45], [-81.83, 41.48],
                [-81.79, 41.49], [-81.75, 41.47], [-81.77, 41.43], [-81.8, 41.42]
              ]]
            }
          },
          {
            'type': 'Feature',
            'properties': {'speed_category': '50-100 Mbps', 'color_code': '#abd9e9'},
            'geometry': {
              'type': 'Polygon',
              'coordinates': [[
                [-81.9, 41.4], [-81.95, 41.45], [-81.92, 41.5],
                [-81.87, 41.52], [-81.84, 41.48], [-81.86, 41.43], [-81.9, 41.4]
              ]]
            }
          },
          {
            'type': 'Feature',
            'properties': {'speed_category': '100+ Mbps', 'color_code': '#74add1'},
            'geometry': {
              'type': 'Polygon',
              'coordinates': [[
                [-81.55, 41.45], [-81.52, 41.48], [-81.5, 41.52], [-81.45, 41.55],
                [-81.4, 41.52], [-81.45, 41.47], [-81.5, 41.45], [-81.55, 41.45]
              ]]
            }
          }
        ]
      ...
      console.log('Broadband data created with ' + broadbandData.features.length + ' features');
      
      // Create the layer but don't add it to the map yet
      var broadbandLayer = L.geoJSON(broadbandData, {
      ...
      // Create legend control but don't add it yet
      var broadbandLegend = L.control({position: 'bottomright'});
      broadbandLegend.onAdd = function(map) {
        var div = L.DomUtil.create('div', 'info legend broadband-legend');
      ...
      // Function to check if a layer is on the map
      ...
      // Function to handle broadband layer toggling
      function updateBroadbandLayerVisibility(showLayer) {
        if (showLayer) {
          if (!isLayerOnMap(broadbandLayer)) {
            broadbandLayer.addTo(map);
            broadbandLegend.addTo(map);
            console.log('Added broadband layer and legend to map');
          }
        } else {
          var removedLayer = safelyRemoveLayer(broadbandLayer);
          var removedLegend = safelyRemoveLayer(broadbandLegend);
          console.log('Removed broadband layer and legend from map:', removedLayer, removedLegend);
        }
      }
      
      // Wait a moment for all Leaflet controls to initialize
      setTimeout(function() {
        // Find the broadband layer checkbox
        var layerControl = document.querySelector('.leaflet-control-layers');
        var broadbandCheckbox = null;
        ...
        if (layerControl) {
          ...
          // Look for the broadband checkbox
          for (var i = 0; i < checkboxes.length; i++) {
            var checkbox = checkboxes[i];
            var label = checkbox.nextElementSibling ? checkbox.nextElementSibling.textContent.trim() : '';
            console.log('Checkbox ' + i + ': ' + label + ' (checked: ' + checkbox.checked + ')');
            
            if (label === 'Broadband Speeds (2021)') {
              broadbandCheckbox = checkbox;
              console.log('Found Broadband Speeds checkbox');
              break;
            }
          }
          
          if (broadbandCheckbox) {
            // First make sure layer is in the correct state based on initial checkbox state
            updateBroadbandLayerVisibility(broadbandCheckbox.checked);
            
            // Now add event listener for future changes
            broadbandCheckbox.addEventListener('change', function(e) {
              console.log('Broadband checkbox changed to:', e.target.checked);
              updateBroadbandLayerVisibility(e.target.checked);
            });
          } else {
            console.warn('Could not find Broadband Speeds checkbox');
          }
        } else {
          console.warn('Layer control not found');
        }
        
        // Ensure any existing broadband layers are removed if not controlled by checkbox
        if (!broadbandCheckbox || !broadbandCheckbox.checked) {
          safelyRemoveLayer(broadbandLayer);
          safelyRemoveLayer(broadbandLegend);
        }
      }, 500); // Wait 500ms for all controls to initialize
    }
  ")
```
